...
def jit_reset(
    ...
    random_params = network_def.init(
        ...
        rngs={"params": online_rng, "dropout": rng},
        support=support,
    )
    target_random_params = network_def.init(
        ...
        actions=actions,
        do_rollout=do_rollout,
        rngs={"params": target_rng, "dropout": rng},
        support=support,
    )
    ...
    online_params = FrozenDict(copy_params(online_params, random_params, keys=keys_to_copy))
    ...
    if reset_target:
        ...
        target_network_params = copy_params(target_network_params, target_random_params, keys=keys_to_copy)
...
def select_action(
    ...
    state = spr_networks.process_inputs(state, rng=rng1, data_augmentation=False, dtype=jnp.float32)
...
def train(
    ...
    def train_one_batch(state, inputs):
        ...
        states = spr_networks.process_inputs(raw_states, rng=rng1, data_augmentation=data_augmentation, dtype=dtype)
        ...
        def loss_fn(
            ...
            if distributional:
                (logits, spr_predictions, _) = get_logits(q_online, current_state, actions[:, :-1], use_spr, batch_rngs)
                logits = jnp.squeeze(logits)
                # Fetch the logits for its selected action. We use vmap to perform this
                # indexing across the batch.
                chosen_action_logits = jax.vmap(lambda x, y: x[y])(logits, actions[:, 0])
                dqn_loss = jax.vmap(losses.softmax_cross_entropy_loss_with_logits)(target, chosen_action_logits)
        ...
        updates, new_optimizer_state = optimizer.update(grad, optimizer_state, params=online_params)
        ...
        if dynamic_scale:
            ...
            online_params = jax.tree_util.tree_map(
                functools.partial(jnp.where, is_fin), new_online_params, online_params
            )
        else:
    ...
    inputs = (
        ...
        raw_next_states.reshape(num_batches, batch_size, *raw_next_states.shape[1:]),
        rewards.reshape(num_batches, batch_size, *rewards.shape[1:]),
        terminals.reshape(num_batches, batch_size, *terminals.shape[1:]),
        same_traj_mask.reshape(num_batches, batch_size, *same_traj_mask.shape[1:]),
        loss_weights.reshape(num_batches, batch_size, *loss_weights.shape[1:]),
        cumulative_gamma.reshape(num_batches, batch_size, *cumulative_gamma.shape[1:]),
    )
...
def create_scaling_optimizer(
    ...
    if name == "adam":
        logging.info(
            ("Creating AdamW optimizer with settings lr=%f, beta1=%f, " "beta2=%f, eps=%f, wd=%f"),
            learning_rate,
            beta1,
            beta2,
            eps,
        ...
        schedule = optax.linear_schedule(0, learning_rate, warmup)
        return optax.inject_hyperparams(optax.rmsprop)(learning_rate=schedule, decay=beta2, eps=eps, centered=centered)
    else:
...
class BBFAgent(dqn_agent.JaxDQNAgent):
    ...
    def __init__(
        ...
        self.shrink_perturb_keys = [s for s in shrink_perturb_keys.lower().split(",") if s]
        ...
        if max_update_horizon is None:
            ...
            n_schedule = exponential_decay_scheduler(cycle_steps, 0, 1, self.update_horizon / self.max_update_horizon)
        ...
        logging.info("\t Found following local devices: %s", str(jax.local_devices()))
        ...
        if min_gamma is None or cycle_steps <= 1:
            ...
            self.gamma_scheduler = exponential_decay_scheduler(cycle_steps, 0, self.min_gamma, self.gamma, reverse=True)

        self.cumulative_gamma = (onp.ones((self.max_update_horizon,)) * self.gamma).cumprod()
    ...
    def _build_networks_and_optimizer(self):
        ...
        self.encoder_mask = FrozenDict({"params": {k: k in encoder_keys for k in self.online_params["params"]}})
        self.head_mask = FrozenDict({"params": {k: k not in encoder_keys for k in self.online_params["params"]}})
        ...
        self.online_params = jax.device_put(self.online_params, jax.local_devices()[0])
        self.target_params = jax.device_put(self.target_network_params, jax.local_devices()[0])
        self.random_params = jax.device_put(self.random_params, jax.local_devices()[0])
        self.optimizer_state = jax.device_put(self.optimizer_state, jax.local_devices()[0])
    ...
    def set_replay_settings(self):
        ...
        self._num_updates_per_train_step = max(1, self._replay_ratio * self.n_envs // self._batch_size)
        self.update_period = max(1, self._batch_size // self._replay_ratio * self.n_envs)
        logging.info(
        ...
        self._num_updates_per_train_step = int(max(1, self._num_updates_per_train_step / self._batches_to_group))
    ...
    def sample_eval_batch(self, batch_size, subseq_len=1):
        self._rng, rng = jit_split(self._rng)
        samples = self._replay.sample_transition_batch(rng, batch_size=batch_size, subseq_len=subseq_len)
        types = self._replay.get_transition_elements()
        replay_elements = collections.OrderedDict()
        for element, element_type in zip(samples, types):
            replay_elements[element_type.name] = element
    ...
    def reset_weights(self):
        ...
        if self.reset_interval_scaling is None or not self.reset_interval_scaling:
            ...
            steps = (
                epochs
                * self._replay.num_elements()
                / (self._batch_size * self._num_updates_per_train_step * self._batches_to_group)
            )
            interval = int(steps) + self.reset_every
        elif isinstance(self.reset_interval_scaling, float) or "." in str(self.reset_interval_scaling):
            interval = self.reset_every * float(self.reset_interval_scaling) ** (self.cumulative_resets)
        else:
        ...
        if self._replay.add_count > self.min_replay_history:
            offline_steps = int(interval * self.offline_update_frac * self._num_updates_per_train_step)
    ...
    def _training_step_update(self, step_index, offline=False):
        ...
        should_log = self.training_steps % self.log_every == 0 and not offline and step_index == 0
        ...
        if self.log_churn and should_log:
            ...
            eval_states = eval_batch["state"].reshape(-1, *eval_batch["state"].shape[-3:])
            eval_actions = eval_batch["action"].reshape(
                -1,
            )
        ...
        if self.training_steps % self.log_every == 0 and not offline and step_index == 0:
            metrics = {
    ...
    def _record_observation(self, observation):
        ...
        if len(observation.shape) == len(self.observation_shape):
            ...
            self._observation = onp.reshape(observation, (observation.shape[0], *self.observation_shape))
    ...
    def delete_one(self, env_id):
        self.state = onp.concatenate([self.state[:env_id], self.state[env_id + 1 :]], 0)
