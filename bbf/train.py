...
flags.DEFINE_string(
    "load_replay_dir",
    None,
    "Directory to load the initial replay buffer from " "a fixed dataset. If None, no transitions are loaded. ",
)
flags.DEFINE_string("tag", None, "Tag for this run")
flags.DEFINE_boolean(
    "save_replay",
    False,
    "Whether to save agent's final replay buffer " "as a fixed dataset to ${base_dir}/replay_logs.",
)
flags.DEFINE_integer(
    "load_replay_number",
    None,
    "Index of the replay run to load the initial "
    "replay buffer from a fixed dataset. If None, uses the `run_number`.",
)
flags.DEFINE_boolean("data_logging", False, "Whether to use agent to log the replay buffer or not.")
flags.DEFINE_boolean("max_episode_eval", True, "Whether to use `MaxEpisodeEvalRunner` or not.")
...
def main(unused_argv):
    ...
    if FLAGS.max_episode_eval:
        kwargs = dict(load_replay_dir=FLAGS.load_replay_dir, save_replay=FLAGS.save_replay)
