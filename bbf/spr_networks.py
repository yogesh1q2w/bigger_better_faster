...
def _per_image_random_crop(key, img, cropped_shape):
    ...
    key_x, key_y = random.split(key, 2)
    x = random.randint(key_x, shape=(batch_size,), minval=0, maxval=img.shape[1] - width)
    y = random.randint(key_y, shape=(batch_size,), minval=0, maxval=img.shape[2] - height)
...
class FeatureLayer(nn.Module):
    ...
    def setup(self):
        if self.noisy:
            self.net = NoisyNetwork(features=self.features, dtype=self.dtype, initializer=self.initializer)
        else:
            self.net = nn.Dense(
                self.features,
...
class ResidualStage(nn.Module):
    ...
    def __call__(self, x, deterministic=None):
        ...
        for _ in range(self.num_blocks):
            ...
            conv_out = nn.Dropout(self.dropout, broadcast_dims=(-3, -2))(conv_out, deterministic)
            conv_out = nn.Conv(
                features=self.dims,
                kernel_size=(3, 3),
                strides=1,
...
class ResNetEncoder(nn.Module):
    ...
    def __call__(self, x, deterministic=None):
        del deterministic
        logging.info(
            "Creating ResNetEncoder with %s stage_sizes, %s num_filters, %s" "norm, %s width_scale, %s strides",
            self.stage_sizes,
            self.num_filters,
            self.norm,
            self.width_scale,
        ...
        if self.norm == "batch":
            norm = functools.partial(
                nn.BatchNorm, use_running_average=False, momentum=0.9, epsilon=1e-5, dtype=self.dtype
            )
        ...
        x = nn.relu(x)
        x = nn.max_pool(x, (3, 3), strides=(self.strides[1], self.strides[1]), padding="SAME")
...
class RainbowDQNNetwork(nn.Module):
    ...
    def setup(self):
        if self.initializer_type == InitializerType.XAVIER_UNIFORM:
            ...
            raise NotImplementedError("Unsupported initializer: {}".format(self.initializer_type))
        ...
        if self.encoder_type == EncoderType.DQN:
            ...
            latent_dim = self.encoder.num_filters * self.width_scale * 2 ** (len(self.encoder.stage_sizes) - 1)
        else:
        ...
        self.predictor = nn.Dense(int(self.hidden_dim), dtype=jnp.float32, kernel_init=initializer)
    ...
    def flatten_spatial_latent(self, spatial_latent, has_batch=False):
        ...
        # logging.info("Flattened representation shape: %s", str(representation.shape))
    ...
    def __call__(
        ...
        if self.distributional:
            ...
            return SPROutputType(q_values, logits, probabilities, spatial_latent, representation)
